...
resource "azurerm_user_assigned_identity" "truefoundry_cert_manager_identity" {
  name                = var.truefoundry_managed_identity_name_override_enabled ? var.truefoundry_managed_identity_override_name : "${var.cluster_name}-cert-manager-managed-identity"
  resource_group_name = var.resource_group_name
